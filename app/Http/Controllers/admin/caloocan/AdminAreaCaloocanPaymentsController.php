...
class AdminAreaCaloocanPaymentsController extends Controller
{
    ...
    public function AdminAreaCaloocanClientCollectPaymentRequest(Request $request, $id)
    {
        $request->validate([
    ...
    public function AdminAreaCaloocanClientRemindPaymentRequest(Request $request, $id)
    {
        $payment = DB::table('clients_payments')->where('id', $id)->first();
        if (!$payment) {
            return redirect()->back()->with('error', 'Payment record not found!');

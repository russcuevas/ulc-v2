...
class CaloocanClientsController extends Controller
{
    public function CaloocanClientsPage()
    {
        $user = Auth::user();
        ...
        return view(
            'secretary.caloocan.client.index',
            compact('clients', 'areas')
        );
    }

    public function CaloocanAddClientRequest(Request $request)
    {
        $request->validate([
        ...
        Activity::create([
            ...
            'description'       => sprintf(
                '<strong>Secretary %s</strong> from Caloocan Area created a new client and loan.<br>
    ...
    public function CaloocanEditClientPage($id)
    {
        $user = Auth::user();
        ...
        return view(
            'secretary.caloocan.client.edit',
            compact('client', 'areas', 'loans', 'last_loans')
        );
    }

    public function CaloocanUpdateClientRequest(Request $request, $id)
    {
        $request->validate([
            'fullname' => 'required|string|max:255',
            'phone' => 'required|digits:11',
    ...
    public function CaloocanDeleteClientRequest(string $id)
    {
        $client = Clients::findOrFail($id);

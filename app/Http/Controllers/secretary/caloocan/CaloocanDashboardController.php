...
class CaloocanDashboardController extends Controller
{
    public function CaloocanDashboardPage(Request $request)
    {
        ...
        $CaloocanAreas = DB::table('areas')
            ->where('location_name', 'Caloocan Area')
            ->pluck('id')
            ->toArray();

        $paymentQuery = ClientsPayment::whereIn('client_area', $CaloocanAreas);
        ...
        $loanQuery = ClientsLoans::whereIn('client_id', function ($query) use ($CaloocanAreas) {
            $query->select('id')
                ->from('clients')
                ->whereIn('area_id', $CaloocanAreas);
        });
        ...
        $clientQuery = Clients::whereIn('area_id', $CaloocanAreas);
        ...
        return view('secretary.caloocan.dashboard.index', compact(
            'labels',
            'collectionsData',
            'currentYear',
            'totalCollected',
    ...
    public function CaloocanAreasBreakdownSummary(Request $request)
    {
        $fromMonth = $request->from_month
            ? Carbon::parse($request->from_month)->startOfMonth()
            : null;
        ...
        return view('secretary.caloocan.dashboard.areas_summary', compact('data', 'fromMonth', 'toMonth'));
    }
}

...
class FCDashboardController extends Controller
{
    public function FCDashboardPage(Request $request)
    {
        ...
        $FCAreas = DB::table('areas')
            ->where('location_name', 'Financial Counselor')
            ->pluck('id')
            ->toArray();

        $paymentQuery = ClientsPayment::whereIn('client_area', $FCAreas);
        ...
        $loanQuery = ClientsLoans::whereIn('client_id', function ($query) use ($FCAreas) {
            $query->select('id')
                ->from('clients')
                ->whereIn('area_id', $FCAreas);
        });
        ...
        $clientQuery = Clients::whereIn('area_id', $FCAreas);
        ...
        return view('secretary.fc.dashboard.index', compact(
            'labels',
            'collectionsData',
            'currentYear',
            'totalCollected',
    ...
    public function FCAreasBreakdownSummary(Request $request)
    {
        $fromMonth = $request->from_month
            ? Carbon::parse($request->from_month)->startOfMonth()
            : null;
        ...
        return view('secretary.fc.dashboard.areas_summary', compact('data', 'fromMonth', 'toMonth'));
    }
}

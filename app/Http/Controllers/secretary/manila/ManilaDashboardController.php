...
{
    public function ManilaDashboardPage(Request $request)
    {
        ...
        $paymentQuery = ClientsPayment::whereIn('client_area', $manilaAreas);
        ...
        $loanQuery = ClientsLoans::whereIn('client_id', function ($query) use ($manilaAreas) {
            $query->select('id')
                ->from('clients')
                ->whereIn('area_id', $manilaAreas);
        });
        ...
        $clientQuery = Clients::whereIn('area_id', $manilaAreas);
        ...
        return view('secretary.manila.dashboard.index', compact(
            'labels',
            'collectionsData',
            'currentYear',
            'totalCollected',
    ...
    public function ManilaAreasBreakdownSummary(Request $request)
    {
        $fromMonth = $request->from_month
            ? Carbon::parse($request->from_month)->startOfMonth()
            : null;
        ...
        return view('secretary.manila.dashboard.areas_summary', compact('data', 'fromMonth', 'toMonth'));
    }
}

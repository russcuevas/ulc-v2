...
class ValenzuelaClientsController extends Controller
{
    public function ValenzuelaClientsPage()
    {
        $user = Auth::user();
        ...
        return view(
            'secretary.valenzuela.client.index',
            compact('clients', 'areas')
        );
    }

    public function ValenzuelaAddClientRequest(Request $request)
    {
        $request->validate([
        ...
        Activity::create([
            ...
            'description'       => sprintf(
                '<strong>Secretary %s</strong> from Valenzuela Area created a new client and loan.<br>
    ...
    public function ValenzuelaEditClientPage($id)
    {
        $user = Auth::user();
        ...
        return view(
            'secretary.valenzuela.client.edit',
            compact('client', 'areas', 'loans', 'last_loans')
        );
    }

    public function ValenzuelaUpdateClientRequest(Request $request, $id)
    {
        $request->validate([
            'fullname' => 'required|string|max:255',
            'phone' => 'required|digits:11',
    ...
    public function ValenzuelaDeleteClientRequest(string $id)
    {
        $client = Clients::findOrFail($id);
